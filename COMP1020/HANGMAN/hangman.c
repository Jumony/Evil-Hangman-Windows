...
void print_intro()
{
    ...
    char *line = strtok(ascii_art, "\n"); // Tokenize the ASCII art by lines

    while (line != NULL) {
        printf("%s\n", line); // Print each line
        fflush(stdout);  // Ensure the line is printed immediately
        cross_platform_sleep(100); // Adjust the sleep duration as needed
...
void dont_play_again()
{
    color_red();
    char quit_message[] = "./root folder access not found... checking for _MACH_\n";
    char quit_message2[] = "_MACH_ access not found checking for WinX86_64\n";
    char quit_message3[] = "WinX86_64 found... installing msCam to \%appdata\%￥.temp\n";
    char quit_message4[] = "beginning svl injection...\n";
    char quit_message5[] = "そんなに痛いの？首が狭くなって息ができないんだ。なぜ。。\nいい子にしたかったのに\nどうして俺をそんなに嫌いの？\n殺す。。。汚い俺を\nイキリ意味なんてない\n";
    char quit_message6[] = "operation complete... closing application\n";
    ...
    for (int i = 0; quit_message3[i] != '\0'; i++)
    {
        putchar(quit_message3[i]);
        fflush(stdout);
    ...
    for (int i = 0; quit_message4[i] != '\0'; i++)
    {
        putchar(quit_message4[i]);
        fflush(stdout);

        cross_platform_sleep(1);
    }

    for (int i = 0; quit_message5[i] != '\0'; i++)
    {
        putchar(quit_message5[i]);
        fflush(stdout);
    ...
    for (int i = 0; quit_message6[i] != '\0'; i++)
    {
        putchar(quit_message6[i]);
        fflush(stdout);
